...
QNetworkReply * FormPostPlugin::postData(QString url) {
    ...
    if (!mUserAttribute.isEmpty()) {
       request.setAttribute(QNetworkRequest::User, (QString)mUserAttribute);
    }
...
void FormPostPlugin::setUserAttribute(QString attr) {
   mUserAttribute = attr;
}

QString FormPostPlugin::getUserAttribute() {
