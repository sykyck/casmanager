...
void CouchDBListener::start()
{
  ...
  QLOG_DEBUG() << QString("%1%2").arg(d->server->secureConnection()).arg(d->server->baseURL());
  ...
  if (netError != QNetworkReply::NoError)
  {
    switch(netError)
    {
      case QNetworkReply::ContentNotFoundError:
        QLOG_ERROR() << QString("Database listener: The content was not found on the server");
        break;

      case QNetworkReply::HostNotFoundError:
        QLOG_ERROR() << QString("Database listener: The server was not found");
        break;

      default:
        QLOG_ERROR() << QString("Database listener: %1").arg(reply->errorString());
        break;
    }
