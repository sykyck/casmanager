...
QString CouchDBServer::baseURL(const bool& withCredential) const
{
  ...
  {
    if (withCredential && hasCredential())
      url = QString("https://%1:%2@%3:%4").arg(d->username, d->password, d->url, QString::number(d->port));
    else
      url = QString("https://%1:%2").arg(d->url, QString::number(d->port));
  }
  else
  {
    if (withCredential && hasCredential())
      url = QString("http://%1:%2@%3:%4").arg(d->username, d->password, d->url, QString::number(d->port));
    else
      url = QString("http://%1:%2").arg(d->url, QString::number(d->port));
  }
